...
/** Внутри раздела «ИИ»: подключение и очередь проверки (M25). */
export function AiTabs({ pending = 0 }: { pending?: number }) {
  const path = useRouterState({ select: (s) => s.location.pathname })
  const tabs = [
    { to: '/service/ai', label: 'Подключение', badge: 0 },
    { to: '/service/ai-review', label: 'На проверке', badge: pending },
  ] as const
