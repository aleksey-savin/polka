  ...
  return (
    <section className="mt-8 first:mt-2">
      ...
      <div className="flex min-h-[150px] items-end gap-[3px] overflow-x-auto px-3.5">
        {books.length === 0 ? (
          <p className="pb-4 text-sm text-muted-foreground">{emptyHint ?? 'На этой полке пока пусто.'}</p>
        ) : (
          books.map((b) => (
            <Spine key={b.id} bookId={b.id} title={b.title} authors={b.authors} pages={b.pages} />
          ))
        )}
      </div>
      <ShelfBoard color={boardColor} />
      {actions && <div className="flex justify-between px-3.5 pt-2">{actions}</div>}
    </section>
  )
}
