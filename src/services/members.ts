...
export async function getMembership(userId: string, libraryId: string): Promise<Membership | null> {
  const rows = await db
    .select()
    .from(libraryMember)
    .where(and(eq(libraryMember.libraryId, libraryId), eq(libraryMember.userId, userId)))
  return rows[0] ?? null
}

/** Доступ к каталогу библиотеки — только для участников. */
export async function assertMember(userId: string, libraryId: string): Promise<Membership> {
  const membership = await getMembership(userId, libraryId)
  if (!membership) throw new AppError('Нет доступа к этой библиотеке', 'forbidden')
  return membership
}

/** Управление библиотекой (участники, удаление) — только создатель. */
export async function assertOwner(userId: string, libraryId: string): Promise<Membership> {
  const membership = await assertMember(userId, libraryId)
  if (membership.role !== 'owner') {
    throw new AppError('Это может сделать только создатель библиотеки', 'forbidden')
  }
  return membership
}
