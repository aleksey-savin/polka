...
export async function listMyTags(
  userId: string,
): Promise<Array<{ id: string; name: string; bookCount: number }>> {
  const rows = await db
...
export async function setBookTags(
  ...
  if (myTagIds.length > 0) {
    await db
      .delete(bookTag)
      .where(and(eq(bookTag.bookId, bookId), inArray(bookTag.tagId, myTagIds)))
  }
  if (wantedIds.length > 0) {
    await db
      .insert(bookTag)
      .values(wantedIds.map((tagId) => ({ bookId, tagId })))
  }
}
