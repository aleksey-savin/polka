...
**Эталонный каталог (M14–M16)** — общий для всех пользователей, правки живут в копиях-книгах. Наполняется автоматически: удачные находки каталогов складывает `persistLookup` — ради этого этап и заводился, повторное добавление той же книги идёт без сети. Записи каталогов модерацию не проходят (в очередь встают только `ref_work`/`ref_book`, заведённые человеком), поэтому в эталон пускаем не всё: транслитные названия (`Deti-bilingvy` вместо «Дети-билингвы») отсеиваются на входе — эталон стоит в цепочке первым, и такая запись портила бы выдачу всем (M32):
...
| `ref_book`        | id, source, sourceRef, isbn13?, isbn10?, title, publisher?, year?, pages?, heightMm?, coverType?, coverPath?, rawJson, checksum? | издание; unique (source, sourceRef). `checksum` (M34) — сумма значимых полей: по ней книги понимают, что запись дополнили |
...
- Порядок опроса с M32 задаётся настройками, а не кодом, и живёт в одном месте
  (`docs/search.md`). Находка каталогов работой ИИ не считается: пометки
  «заполнил ИИ» не получает и в очередь модератора не идёт — это обычное
  дозаполнение.
...
- Ядро с M32 — `findEdition` (`services/find/`), `recognizeBook` осталась
  обёрткой с проверкой доступа. Ступени модели по памяти нет с M30.1. Человек
  отверг показанное — `nextVariant` дописывает ключ варианта в
  `ai_isbn_guess.rejected_vias`, и цепочка идёт дальше; отвергнутое из кэша не
  отдаётся. Подробности цепочки — `docs/search.md`.
- Добор недостающего (обложка, аннотация, объём) идёт по той же цепочке и тем
  же настройкам: `enrichDraft` спрашивает те ступени, у которых он есть.
- Находка каталогов при сохранении НЕ получает пометку
...
- Черновик с M34 содержит **полное издание**: кроме названия, авторов,
  издательства и года — объём, язык, серия, аннотация и обложка. Раньше их было
  нечем занести, и следующий владелец получал такую же куцую карточку.
  При публикации книги с этим ISBN получают `ref_book_id`, а у книги
  появляется `ref_checksum` — версия эталона, применённая в карточку.
...
- В очередь попадают не только жалобы: с M32 туда встают записи эталона на
  **проверку актуальности** (латиница вместо русского названия или отказ
  владельца от этих данных). Зачем поставили — в поле `reason`.
...
- Ступени «спросить модель по памяти» нет (M30.1): ISBN — случайное число, и
  ...
  Модель работает там, где полезна: читает страницы, найденные веб-поиском.
